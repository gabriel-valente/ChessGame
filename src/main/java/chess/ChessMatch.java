...
public class ChessMatch {
    ...
    public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
        ...
        if (movedPiece instanceof Pawn && Math.abs(target.getRow() - source.getRow()) == 2) enPassantVulnerable = movedPiece;
    ...
    private void validateSourcePosition(Position position) {
        ...
        if (currentPlayer != ((ChessPiece)board.piece(position)).getColor()) throw new ChessException("The selected piece isn't yours");
        if (!board.piece(position).isThereAnyPossibleMove()) throw new ChessException("There are no moves for the selected piece");
    }

    private void validateTargetPosition(Position source, Position target) {
        if (!board.piece(source).possibleMove(target)) throw new ChessException("The selected piece cannot move to the target position");
    }
